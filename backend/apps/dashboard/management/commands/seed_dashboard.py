...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        admin, created = User.objects.get_or_create(
            username="admin", email="admin@atelier.com"
        )
        ...
        admin.save()
        self.stdout.write(
            f"  Admin: {admin.username} ({'created' if created else 'updated'})"
        )
        ...
        lessons_data = [
            (
                ...
                "intro-os",
                "Welcome to open source. Learn issues, forks, clones, and etiquette.",
                10,
            ),
            (
                "Mastering local Git branches",
                "git-branches",
                "Deep dive into local branch models, HEAD staging.",
                15,
            ),
            (
                "Making pull requests with ease",
                "make-prs",
                "Create branch, make changes, commit, push, and open pull request.",
                15,
            ),
            (
                "Resolving scary conflicts",
                "git-conflicts",
                "Learn how to read three-way diff marks and safely resolve conflicts.",
                20,
            ),
            (
                "Clean commits & Rebase flows",
                "git-rebase",
                "Practice safe commit squashing, interactive rebases, and clean trees.",
                25,
            ),
        ]
        lessons = []
        ...
        for contributor in contributors:
            ...
            for idx in completed_indices:
                ...
                lp, _ = LessonProgress.objects.get_or_create(
                    user=contributor, lesson=lesson
                )
                lp.completed = True
        ...
        self.stdout.write(
            f"  Certificate for Alice: {cert.verification_hash} ({'created' if cert_created else 'updated'})"
        )
        ...
        issues_data = [
            (
                "Fix dark mode contrast in forms",
                "Make forms readable in dark mode.",
                Issue.Status.SOLVED,
                50,
                "alice",
            ),
            (
                "Implement simple token caching on client",
                "Avoid extra fetch calls by caching tokens in localStorage.",
                Issue.Status.SOLVED,
                100,
                "alice",
            ),
            (
                "Add validation schema for user signup",
                "Ensure usernames are safe alphanumeric characters.",
                Issue.Status.IN_PROGRESS,
                75,
                "bob",
            ),
            (
                "Write mock endpoints for profile sandbox",
                "Prepare mock structures to emulate shell operations.",
                Issue.Status.OPEN,
                60,
                "bob",
            ),
            (
                "Fix broken link in Navigation sidebar",
                "Update old relative path to dashboard routes.",
                Issue.Status.SOLVED,
                30,
                "charlie",
            ),
            (
                "Refactor dashboard widgets to grid structure",
                "Improve responsiveness of analytics cards.",
                Issue.Status.OPEN,
                80,
                None,
            ),
            (
                ...
                "Add section in wiki explaining compose production setups.",
                Issue.Status.OPEN,
                50,
                None,
            ),
        ...
        for title, desc, status, points, assigned_username in issues_data:
            assigned_user = (
                User.objects.filter(username=assigned_username).first()
        ...
        prs_data = [
            (
                "feat: solve dark mode contrast issues",
            ...
            (
                "feat: add token localStorage cache",
                PullRequest.Status.MERGED,
                issues[1],
                "alice",
                2,
            ...
            (
                "docs: fix navigation links to dashboard",
                PullRequest.Status.MERGED,
                issues[4],
                "charlie",
                3,
            ...
            (
                "feat: add signup regex validator",
            ...
            (
                "chore: update README deploy instructions",
            ...
            (
                "feat: dockerize atelier container stack",
                PullRequest.Status.OPEN,
                None,
                "bob",
                0,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Demo credentials: Username: 'admin' or 'alice' or 'bob' · Password: 'password123'"
            )
        )
