...
class PDFReportGenerator:
    ...
    def generate(self) -> bytes:
        ...
        # 1. Header
        elements.append(Paragraph("User Progress & Achievement Report", self.styles["ReportTitle"]))
        
        gen_date = timezone.now().strftime("%Y-%m-%d %H:%M")
        elements.append(Paragraph(f"<b>User:</b> {self.user.username}", self.styles["NormalText"]))
        elements.append(Paragraph(f"<b>Email:</b> {self.user.email}", self.styles["NormalText"]))
        elements.append(Paragraph(f"<b>Generated on:</b> {gen_date}", self.styles["NormalText"]))
        ...
        completed_lessons_count = LessonProgress.objects.filter(user=self.user, completed=True).count()
        ...
        lessons = LessonProgress.objects.filter(user=self.user).select_related("lesson").order_by("-updated_at")[:20]
        
        if lessons:
            lesson_data = [["Lesson Name", "Status", "Score", "Last Attempt"]]
            for lp in lessons:
                status = "Completed" if lp.completed else "In Progress"
                date_str = lp.updated_at.strftime("%Y-%m-%d") if lp.updated_at else "N/A"
                lesson_data.append([
                    lp.lesson.title[:40] + ("..." if len(lp.lesson.title) > 40 else ""),
                    status,
                    str(lp.score),
                    date_str
                ])
            ...
            elements.append(Paragraph("No learning progress recorded yet.", self.styles["NormalText"]))
        elements.append(Spacer(1, 20))

        # 4. Achievements & Badges
        elements.append(Paragraph("Achievements & Badges", self.styles["SectionHeader"]))
        badges = UserBadge.objects.filter(user=self.user).select_related("badge").order_by("-earned_at")
        ...
        if badges:
            ...
            elements.append(Paragraph("No badges earned yet.", self.styles["NormalText"]))
        elements.append(Spacer(1, 20))

        # 5. Certificates
        elements.append(Paragraph("Certifications", self.styles["SectionHeader"]))
        certificates = Certificate.objects.filter(user=self.user, is_active=True).order_by("-issued_at")
        ...
        if certificates:
            ...
            elements.append(Paragraph("No active certificates found.", self.styles["NormalText"]))
        elements.append(Spacer(1, 20))

        # 6. Coding Activity
        elements.append(Paragraph("Coding Activity", self.styles["SectionHeader"]))
        submissions = CodeSubmission.objects.filter(user=self.user).order_by("-created_at")[:10]
        ...
        if submissions:
            ...
            elements.append(Paragraph("No code submissions found.", self.styles["NormalText"]))
