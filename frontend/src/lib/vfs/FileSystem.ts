...
export class VirtualFileSystem {
  ...
  static resolvePath(cwd: string[], path: string): string[] {
    ...
    for (const part of parts) {
      if (part === "~") { result.length = 0; result.push("~"); }
  ...
  static setNode(state: VfsState, pathParts: string[], node: VfsNode | null): VfsState {
    if (pathParts.length === 1 && pathParts[0] === "~") {
      if (!node || node.type !== "dir") throw new VfsError("Cannot overwrite root directory");
  ...
  static mkdir(state: VfsState, path: string): VfsState {
    ...
    if (this.getNode(state, resolved)) throw new VfsError(`mkdir: ${path}: already exists`);
    
    // Ensure parent exists
    const parentPath = resolved.slice(0, -1);
    const parent = this.getNode(state, parentPath);
    if (!parent || parent.type !== "dir") throw new VfsError(`mkdir: ${path}: No such file or directory`);

    const now = Date.now();
    return this.setNode(state, resolved, { type: "dir", children: {}, created: now, modified: now });
  }
  ...
  static touch(state: VfsState, path: string): VfsState {
    ...
    const parentPath = resolved.slice(0, -1);
    const parent = this.getNode(state, parentPath);
    if (!parent || parent.type !== "dir") throw new VfsError(`touch: ${path}: No such file or directory`);

    return this.setNode(state, resolved, { type: "file", content: "", size: 0, created: now, modified: now });
  }
  ...
  static write(state: VfsState, path: string, content: string): VfsState {
    ...
    if (existing && existing.type === "dir") throw new VfsError(`write: ${path}: is a directory`);

    return this.setNode(state, resolved, { type: "file", content, size: content.length, created: existing ? (existing as VfsFile).created : now, modified: now });
  }

  static rm(state: VfsState, path: string, recursive: boolean): VfsState {
    const resolved = this.resolvePath(state.cwd, path);
    if (resolved.length === 1 && resolved[0] === "~") throw new VfsError(`rm: cannot remove root directory`);
    
    const target = this.getNode(state, resolved);
    if (!target) throw new VfsError(`rm: ${path}: No such file or directory`);
    
    if (target.type === "dir" && !recursive) throw new VfsError(`rm: ${path}: is a directory`);
  ...
  static cp(state: VfsState, src: string, dest: string, recursive: boolean): VfsState {
    ...
    if (srcNode.type === "dir" && !recursive) throw new VfsError(`cp: -r not specified; omitting directory '${src}'`);
    ...
    if (destStr === srcStr || destStr.startsWith(srcStr + "/")) {
      throw new VfsError(`cp: cannot copy '${src}' to a subdirectory of itself`);
    }
  ...
  static mv(state: VfsState, src: string, dest: string): VfsState {
    ...
    const destStr = destResolved.join("/");
    if (destStr === srcStr || destStr.startsWith(srcStr + "/")) {
      throw new VfsError(`mv: cannot move '${src}' to a subdirectory of itself`);
    }
